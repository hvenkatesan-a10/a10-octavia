...
class GetVThunderTask(BaseDatabaseTask):
    ...
    def execute(self, amphora):
        with db_apis.session().begin() as session:
            vthunder = self.vthunder_repo.get(session, id=123)
        return vthunder
...
class GetVThunderAmphora(BaseDatabaseTask):
    ...
    def execute(self, vthunder):
        ...
        with db_apis.session().begin() as session:
            amphora = self.amphora_repo.get(session,
...
class GetBackupVThunderByLoadBalancer(BaseDatabaseTask):
    ...
    def execute(self, loadbalancer, vthunder=None, flag=False):
        loadbalancer_id = loadbalancer[constants.LOADBALANCER_ID]
        with db_apis.session().begin() as session:
            barbican_client = BarbicanACLAuth().get_barbican_client(loadbalancer.get(constants.PROJECT_ID))
...
class CreateVThunderHealthEntry(BaseDatabaseTask):
    ...
    def execute(self, loadbalancer, vthunder_config):
        with db_apis.session().begin() as session:
            self.vthunder_repo.create(session,
                                    vthunder_id=uuidutils.generate_uuid(),
                                    device_name=vthunder_config.device_name,
                                    username=vthunder_config.username,
...
class UpdateVRIDForLoadbalancerResource(BaseDatabaseTask):
    ...
    def revert(self, *args, **kwargs):
        for vrid in self.vrid_created:
            try:
                with db_apis.session().begin() as session:
                    self.vrid_repo.delete(session, id=vrid.id)
            except Exception as e:
...
class CheckVLANCanBeDeletedParent(object):
    ...
    def is_vlan_deletable(self, project_id, subnet_id, is_vip):
        ...
        lbs = session.query(self.loadbalancer_repo.model_class).filter(
...
#             session, project_id=project_id, subnet_id=subnet.id)
#         return vrid
            ...
            try:
                ...
                with db_apis.session().begin() as session:
                    self.nat_pool_repo.create(
                        session, id=id, name=pool_name, subnet_id=subnet_id,
            ...
            try:
                ...
                with db_apis.session().begin() as session:
                    self.nat_pool_repo.update(
                        session, nat_pool.id, member_ref_count=ref)
            except Exception as e:
...
class DeleteNatPoolEntry(BaseDatabaseTask):
    ...
    def execute(self, nat_pool=None):
        ...
        if nat_pool.member_ref_count > 1:
            ...
            try:
                with db_apis.session().begin() as session:
                    self.nat_pool_repo.delete(session, id=nat_pool.id)
        ...
        try:
            ...
            with db_apis.session().begin() as session:
                ...
                if entry is None:
                    ...
                    entry = self.vrrp_set_repo.get(session, project_id=project_id)

                if entry is not None:
                    return entry.set_id

                subnet_set_ids = self.vrrp_set_repo.get_subnet_set_ids(session,
                                                                    mgmt_subnet)
                ...
                self.vrrp_set_repo.create(
                    session, mgmt_subnet=mgmt_subnet, project_id=project_id,
                    set_id=set_id)
...
class GetProjectVRRPSetId(BaseDatabaseTask):
    ...
    def execute(self, vthunder):
        ...
        try:
            ...
            with db_apis.session().begin() as session:
                entry = self.vrrp_set_repo.get(session,
        ...
        with db_apis.session().begin() as session:
            ...
            for vthunder_id in vthunder_ids:
                ...
                lb = self.loadbalancer_repo.get_lb_excluding_deleted(
                    session, vthunder.loadbalancer_id)
                if lb:
                    amphora = self.amphora_repo.get(session, load_balancer_id=lb.id)
                    if not amphora:
                        """
...
class DeleteStaleSpareVThunder(BaseDatabaseTask):
    ...
    def execute(self, spare_vthunder):
        if spare_vthunder:
            ...
            try:
                with db_apis.session().begin() as session:
                    self.amphora_repo.delete(session, id=spare_vthunder.amphora_id)
            except Exception as e:
                LOG.exception("Failed to delete used spare amphora %s due to %s",
                              spare_vthunder.amphora_id, str(e))
            try:
                with db_apis.session().begin() as session:
                    self.vthunder_repo.delete(session, id=spare_vthunder.id)
            except Exception as e:
            ...
            with db_apis.session().begin() as session:
                ...
                if blade:
                    try:
                        ...
                        self.vthunder_repo.update(session, blade.id,
                                                role=constants.ROLE_MASTER)
                        self.vthunder_repo.update(session, vthunder.id,
...
class FailoverPostDbUpdate(BaseDatabaseTask):
    ...
    def execute(self, vthunder, spare_vthunder):
        ...
        try:
            with db_apis.session().begin() as session:
                spare_amphora = self.amphora_repo.get(session,
...
class CountLBThunderPartition(BaseDatabaseTask):
    def execute(self, vthunder):
        ...
        with db_apis.session().begin() as session:
            return self.vthunder_repo.get_lb_count_vthunder_partition(session,
                                                                    vthunder.ip_address,
                                                                    vthunder.partition_name)
